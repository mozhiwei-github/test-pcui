import os
import pytest
from common.contants import EnvVar
from common.yaml_reader import YamlReader

"""UI自动化测试启动脚本"""


def run_pytest(case_list, serve, generate, smb, channel, env, tod):
    ...
    if channel:
        pytest_args.append(f"--channel={channel}")

    if env:
        pytest_args.append(f"--env={env}")

    if tod:
        pytest_args.append(f"--tod={tod}")
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser("For interface test")
    parser.add_argument("case", help="测试用例文件路径，多个文件用英文逗号分隔。")
    parser.add_argument("-s", "--serve", action='store_true',
                        help="是否以allure服务形式查看报告。（需本机已安装allure命令）")
    parser.add_argument("-g", "--generate", action='store_true',
                        help="是否生成allure html报告。（需本机已安装allure命令）")
    parser.add_argument("--smb", help="smb地址，使用smb中文件替换毒霸相应文件")
    parser.add_argument("--channel", help="测试渠道")
    parser.add_argument("--env", help="测试环境")
    parser.add_argument("--tod", help="安装包tod")
    ...
    yaml_result = reader.read_yaml(os.path.join(os.getcwd(), "common", "config", "common.yml"))
    # 设置日志级别环境变量
    ...
    run_pytest(run_case_list, args.serve, args.generate, args.smb, args.channel, args.env, args.tod)
