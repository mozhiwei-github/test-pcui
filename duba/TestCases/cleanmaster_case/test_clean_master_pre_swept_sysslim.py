...
@allure.epic(f'清理大师, c盘瘦身-预扫泡测试（{config.ENV.value}）')
class TestduliKsysslimInterfacePackage(object):
    @allure.story('1.清理大师, c盘瘦身-预扫泡测试')
    def test_ksysslim_base(self):
        allure.dynamic.description(
            '\t1.c盘瘦身-预扫泡检查\n'
        )
        allure.dynamic.title('c盘瘦身-预扫泡测试')
        ...
        with allure.step("step1：c盘瘦身预扫泡检查"):
            pre_swept.install_reg_remove()
            ...
            assert retpa, log.log_error("c盘瘦身预扫泡无弹出", need_assert=False)
            log.log_pass("c盘瘦身预扫泡，弹出功能正常")
