...
@allure.epic(f'清理大师, 软件卸载-预扫泡测试（{config.ENV.value}）')
class TestduliKsysslimInterfacePackage(object):
    @allure.story('1.清理大师, 软件卸载-预扫泡测试')
    def test_ksysslim_base(self):
        allure.dynamic.description(
            '\t1.软件卸载-预扫泡检查\n'
        )
        allure.dynamic.title('软件卸载-预扫泡测试')
        ...
        with allure.step("step1：软件卸载预扫泡检查"):
            pre_swept.install_reg_remove()
            pre_swept.popdata_pop_config(defrag_switch="0")
            pre_swept.kvipapp_setting_pop_cconfig(this_pop_name="SoftUninstallMgr_NormalPop")
            pre_swept.restart_cmcore()
            retpa = pre_swept.close_preswept_kfixstar_pop(name="软件卸载")
            assert retpa, log.log_error("软件卸载预扫泡弹出失败", need_assert=False)
            log.log_pass("软件卸载预扫泡，弹出功能正常")
