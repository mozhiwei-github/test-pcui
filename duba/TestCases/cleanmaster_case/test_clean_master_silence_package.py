...
from duba.PageObjects.clean_master_install_page import KcleanMasterInstall, InstallCheck, DownloadPackage, \
    check_package_slience_installing, print_tid_tod
...
class TestduliKsysslimInterfacePackage(object):
    ...
    def test_ksysslim_base(self):
        ...
        allure.dynamic.title('清理大师静默安装包安装卸载流程测试')
        ...
        with allure.step("step3：安装过程检查"):
            res = check_package_slience_installing()
            assert res, "安装过程异常"
            log.log_pass("安装过程正常")
            perform_sleep(1)
