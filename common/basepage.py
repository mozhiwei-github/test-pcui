...
class BasePage:
    ...
    def __init__(self, do_pre_open=True, page_desc=None, delay_sec=0, is_monitor_perf=True, check_open_result=True):
        ...
        # 加入进程监控
        if self.process_name and is_monitor_perf:
            utils.add_process_monitor(self.process_name)
    ...
    def page_close(self):
        ...
        if self.count_page_close < 4 and self.page_is_exist():  # 这里控制page_close只进来重试执行一次
